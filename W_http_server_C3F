...
// 开关性 预定义宏
//#define RECORD_IN_FILE  //宏定义后启用记录文件，否则仅显示于控制台 
#define IP_PROTOCOL_IPv4_6  //默认未定义既IPv4，定义生效既IPv4与6

// 用户配置 参数性 预定义宏
#define DEFAULT_PORT 80u //监听端口
#define MAX_CONNECT_NUMBER 20u //最大连接数
...
//服务器 相应内存配置 参数性 预定义宏
#define RESPOND_CHUNK_ENCODING_THRESHOLD 1024000u //返回内容超过此阈值则采用chunk编码以节省内存空间(实测回应时间会延长很多) 单位Byte

//服务器 核心 储存数据结构 内存配置 参数性 预定义宏
...
#endif
#define printw(content) print_and_record_with_time(content) //打印并记录(带有时间戳)
...
//传参用句柄与套接字组合结构体
struct param_hostname_and_socket
{
...
* @param client 客户套接字
* @param client_name 客户主机名
* @return 如果是HTTP/1.1返回1 否则0
*/
u2 HTTP_connection_handle(const SOCKET client, const char* client_name)
{
	u2 keep_alive = 0;
	...
	{
		char rec[128];
		sprintf(rec, "客户[%s]:\t对方关闭连接，正在清理并退出\r\n", client_name);
		printw(rec);
		return 0;
	}
	if (recv_ret == SOCKET_ERROR)
	{
		char rec[128];
		sprintf(rec,"客户[%s]:\tHTTP接收错误: WSA_err_code %d \r\n", client_name, WSAGetLastError());
		printw(rec);
		return 0;
	}
	...
	{
		char rec[128];
		sprintf(rec, "客户[%s]:\tHTTP 请求方法错误，已终结该连接\r\n", client_name);
		printw(rec);
		return 0;
	}
	strpos += paclen + 1;
	...
	{
		char rec[128];
		sprintf(rec, "客户[%s]:\tHTTP URL　长度超过阈值，已终结该连接\r\n", client_name);
		printw(rec);
		//414 url too large
	...
	{
		char rec[128] = { 0 };
		sprintf(rec, "客户[%s]:\tHTTP version: |%s|　版本超前，无法支持，已关闭连接\r\n", client_name, version);
		printw(rec);
	...
	{
		char rec[128];
		sprintf(rec, "客户[%s]:\tHTTP version　长度超过阈值，已终结该连接\r\n", client_name);
		printw(rec);
	...
	char rec[128 + MAXLEN_HTTP_URL];
	sprintf(rec, "客户[%s]:\t 方法:|%s|\turl:|%s|\r\n", client_name, method, url);
	printw(rec);
...
DWORD WINAPI HTTP_connection_handle_thread(const LPVOID lparam)
{
	...
	u2 hold_link = 1;
	char rec[128];
	struct param_hostname_and_socket* lpparam = lparam;
	sprintf(rec, "客户主机名:%s\t开始\t当前连接数:%d\n", lpparam->host_name, lpparam->client_socket, thread_total_num);
	printw(rec);
	while (hold_link)
	{
		hold_link = HTTP_connection_handle(lpparam->client_socket, lpparam->host_name);
	}
	closesocket(lpparam->client_socket);
	memset(rec, 0, 128);
	thread_total_num--;
	sprintf(rec, "客户主机名:%s\t结束\t当前连接数：%d\n", lpparam->host_name, lpparam->client_socket, thread_total_num);
	printw(rec);
	free(lpparam);
}

#ifdef IP_PROTOCOL_IPv4_6//双兼容IPv46
//make from microsoft developer document https://learn.microsoft.com/en-us/windows/win32/winsock/ipv6-enabled-server-code-2
//修改时切除了其UDP部分，仅保留了TCP处理方法 地址端口等在getaddrinfo处修改
void HTTP_server_loop()
{
	...
	if ((RetVal = WSAStartup(MAKEWORD(2, 2), &wsaData)) != 0) {
		print_and_record("WSAStartup failed with error %d: %s\n", RetVal, PrintError(RetVal));
		WSACleanup();
		return;
	}
	...
	Hints.ai_family = AF_UNSPEC;//AF_UNSPEC未指定 AF_INET仅IPv4 AF_INET6仅IPv6
	Hints.ai_socktype = SOCK_STREAM;//SOCK_STREAM既TCP SOCK_DGRAM既UDP
	Hints.ai_flags = AI_NUMERICHOST | AI_PASSIVE;
	RetVal = getaddrinfo(NULL, "http", &Hints, &AddrInfo);
	if (RetVal != 0) {
		print_and_record("getaddrinfo failed with error %d: %s\n", RetVal, gai_strerror(RetVal));
		WSACleanup();
		return -1;
	}
	//
	...
	//
	u4 i = 0;
	for ( AI = AddrInfo; AI != NULL; AI = AI->ai_next,i++)
	{
		// Highly unlikely, but check anyway.
		if (i == FD_SETSIZE) {
			print_and_record("getaddrinfo returned more addresses than we could use.\n");
			break;
		}
		...
		if (ServSock[i] == INVALID_SOCKET) {
			print_and_record("socket() failed with error %d: %s\n", WSAGetLastError(), PrintError(WSAGetLastError()));
			continue;
		}
		...
		if ((AI->ai_family == PF_INET6) &&
			...
			print_and_record("IPv6 link local addresses should specify a scope ID!\n");
		}
		//
		...
		if (bind(ServSock[i], AI->ai_addr, (int)AI->ai_addrlen) == SOCKET_ERROR) {
			print_and_record("bind() failed with error %d: %s\n", WSAGetLastError(), PrintError(WSAGetLastError()));
		...
		// ** 这里删除了if socket type == SOCK_STREAM 才执行下面listen代码 **
		if (listen(ServSock[i], 5) == SOCKET_ERROR) {
			print_and_record("listen() failed with error %d: %s\n", WSAGetLastError(), PrintError(WSAGetLastError()));
			closesocket(ServSock[i]);
			continue;
		}

		print_and_record("listen: addr:%s,\tport: 80(http),\tprotocal:TCP\n");
	}
	freeaddrinfo(AddrInfo);

	if (i == 0) {
		print_and_record("Fatal error: unable to serve on any address.\n");
		WSACleanup();
		return -1;
	}
	NumSocks = i;
	...
	while (1)
	{
		...
		for (i = 0; i < NumSocks; i++) {
			if (FD_ISSET(ServSock[i], &SockSet))  //判断是否有仍未处理的可读
				break;
		}
		if (i == NumSocks) {
			for (i = 0; i < NumSocks; i++)
				FD_SET(ServSock[i], &SockSet);
			if (select(NumSocks, &SockSet, 0, 0, 0) == SOCKET_ERROR)  //清空不可读，留下可读
			{
				print_and_record("select() failed with error %d: %s\n", WSAGetLastError(), PrintError(WSAGetLastError()));
				WSACleanup();
				return -1;
			}
		}
		for (i = 0; i < NumSocks; i++) 
		{
			if (FD_ISSET(ServSock[i], &SockSet))  //挑出可读
			{
		...
		{
			//第七步，创建线程接受浏览器请求
			struct param_hostname_and_socket* lpparam = (struct param_hostname_and_socket*)malloc(sizeof(struct param_hostname_and_socket));
			lpparam->client_socket = ConnSock;

			//获取对方主机名
			if (getnameinfo((LPSOCKADDR)&From, FromLen, lpparam->host_name, sizeof(lpparam->host_name), NULL, 0, NI_NUMERICHOST) != 0)
				memcpy(lpparam->host_name, 10, "<unknown>");
		...
		else
		{
			...
			HTTP_respond(ConnSock, 503, 0, 0, 0, 0);
